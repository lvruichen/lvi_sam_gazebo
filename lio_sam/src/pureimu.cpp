...
public:
  ...
  const double imuAccBiasN = 2e-4;
  const double imuGyrBiasN = 3e-5;
  const double imuGravity = 9.80;
  ...
  gtsam::PreintegratedImuMeasurements *imuIntegratorGt_;
  ...
  // imu to base_footprint    for husky
  gtsam::Pose3 imu2Lidar =
      gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(0, 0, 0.13228));
  gtsam::Pose3 lidar2Imu =
      gtsam::Pose3(gtsam::Rot3(1, 0, 0, 0), gtsam::Point3(0, 0, -0.13228));

  // ax gx
  //   gtsam::imuBias::ConstantBias gt_imu_bias;

  IMUPreintegration() {
    subImu = nh.subscribe<sensor_msgs::Imu>(
        "/imu/data", 2000, &IMUPreintegration::imuHandler, this,
        ros::TransportHints().tcpNoDelay());
    subOdometry = nh.subscribe<nav_msgs::Odometry>(
        "/odom", 100, &IMUPreintegration::odometryHandler, this,
        ros::TransportHints().tcpNoDelay());

    pubImuOdometry = nh.advertise<nav_msgs::Odometry>("odometry/imu", 2000);
    pubImuPath = nh.advertise<nav_msgs::Path>("/imu/path", 1);
    pubPosePath = nh.advertise<nav_msgs::Path>("/pose/path", 1);
    ...
    p->accelerometerCovariance = gtsam::Matrix33::Identity(3, 3) *
                                 pow(1e-4, 2); // acc white noise in continuous
    p->gyroscopeCovariance = gtsam::Matrix33::Identity(3, 3) *
                             pow(1e-4, 2); // gyro white noise in continuous
    ...
    // gt_imu_bias = gtsam::imuBias::ConstantBias();

    priorPoseNoise = gtsam::noiseModel::Diagonal::Sigmas(
        (gtsam::Vector(6) << 3e-1, 3e-1, 3e-1, 1e-2, 1e-2, 5e-2)
            .finished()); // rad,rad,rad,m, m, m
    priorVelNoise = gtsam::noiseModel::Isotropic::Sigma(3, 1e-2); // m/s
    priorBiasNoise = gtsam::noiseModel::Isotropic::Sigma(
        6, 1e-6); // 1e-2 ~ 1e-3 seems to be good
    correctionNoise = gtsam::noiseModel::Isotropic::Sigma(6, 2e-2); // meter
    ...
    // imuIntegratorGt_ = new gtsam::PreintegratedImuMeasurements(
    //     p, gt_imu_bias); // setting up the IMU integration for optimization
  }
  ...
  void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg) {
    ...
    static gtsam::Pose3 prevPoseNotOptim_;
    ...
    // if(!imuQueOpt.empty()){
    //     sensor_msgs::Imu *thisImu = &imuQueOpt.back();
    //     double diff = currentCorrectionTime - ROS_TIME(thisImu);
    //     ROS_WARN_STREAM_COND(diff > 0, "diff: " << diff);
    // }

    // 1. integrate imu data and optimize
    ROS_WARN_STREAM("imuqueopt(" << imuQueOpt.size() << ")");
    ROS_WARN_STREAM("imuqueimu(" << imuQueImu.size() << ")");
    while (!imuQueOpt.empty()) {
    ...
    // correctionNoise); graphFactors.add(pose_factor);

    // try to add relative odom factor
    gtsam::Pose3 relativeOdom = prevPoseNotOptim_.inverse() * curPose;
    graphFactors.add(gtsam::BetweenFactor<gtsam::Pose3>(
        X(key - 1), X(key), relativeOdom, correctionNoise));

    prevPoseNotOptim_ = curPose;
    ...
    // visualize the optimized pose
    // pubPosePathFun(prevPose_);
    ...
    // repropogate
    if (!imuQueImu.empty()) {
      ROS_WARN_STREAM("reprogate imu integration( " << imuQueImu.size()
                                                    << " )");
      std::cout << "prev bias: " << prevBiasOdom << std::endl;
      ...
      imuIntegratorImu_->resetIntegrationAndSetBias(prevBiasOdom);
      ROS_WARN("imu deque is empty!! no need to reprogate!!");
    }
  ...
  void imuHandler(const sensor_msgs::Imu::ConstPtr &imuMsg) {
    ...
    //     map_to_odom, thisImu.header.stamp, "world", "odom"));
    ...
    // size_t resetNum = 500 * 50;
    // static gtsam::NavState firstState(gtsam::Rot3(1, 0, 0, 0),
    //                                   gtsam::Point3(0, 0, 0),
    //                                   gtsam::Velocity3(0, 0, 0));
    // static size_t resetCnt = 0;
    // imuIntegratorGt_->integrateMeasurement(
    //     gtsam::Vector3(thisImu.linear_acceleration.x,
    //                    thisImu.linear_acceleration.y,
    //                    thisImu.linear_acceleration.z),
    //     gtsam::Vector3(thisImu.angular_velocity.x,
    ...
    // gtsam::NavState pureImuState =
    //     imuIntegratorGt_->predict(firstState, gt_imu_bias);
    // if (resetCnt++ >= resetNum) {
    //   resetCnt = 0;
    //   firstState = pureImuState;
    //   imuIntegratorGt_->resetIntegration();
    // }

    // publish pure imu transformation
    // pubTF(pureImuState.pose(), "pureImuLink", thisImu.header.stamp);

    // predict odometry
    gtsam::NavState currentState =
        imuIntegratorImu_->predict(prevStateOdom, prevBiasOdom);

    gtsam::NavState IdentWithVel(gtsam::Pose3::identity(),
                                 prevStateOdom.velocity());
    gtsam::NavState imuIntegState =
        imuIntegratorImu_->predict(IdentWithVel, prevBiasOdom);
    // pubTF(imuIntegState.pose(), "imuIntegIncLink", thisImu.header.stamp);

    // test
    // gtsam::Pose3 currentStatePose = currentState.pose();
    // gtsam::Pose3 imuIntegStatePose = imuIntegState.pose();
    // gtsam::Pose3 diffPose = currentStatePose * imuIntegStatePose.inverse();
    // // check diffPose and prevStateOdom
    // static double lastCheckTime = ros::Time::now().toSec();
    // double diffCheckTime = ros::Time::now().toSec() - lastCheckTime;
    // if(diffCheckTime > 2.0){
    //     lastCheckTime += diffCheckTime;
    //     std::cout << " check diffPose and prevStateOdom : " <<
    //     diffPose.inverse() * prevStateOdom.pose() << std::endl;
    // }
    ...
    //     tf::StampedTransform(tCur, thisImu.header.stamp, "odom", "loc_link");
    // tfOdom2BaseLink.sendTransform(odom_2_baselink);
  }

  // debug
  void pubTF(gtsam::Pose3 const &pose, std::string child_link,
             ros::Time const &stamp) {
    ...
    tf::Transform tf_to_pub;
    tf::poseMsgToTF(odom.pose.pose, tf_to_pub);
    tf::StampedTransform stf_to_pub =
        tf::StampedTransform(tf_to_pub, stamp, "odom", child_link);
    tfOdom2BaseLink.sendTransform(stf_to_pub);
  }

  void pubPosePathFun(gtsam::Pose3 const &pose) {
    nav_msgs::Odometry odom;
    ...
    posePath.poses.push_back(pose_stamped);

    if (posePath.poses.size() >= 5000)
      posePath.poses.erase(posePath.poses.begin());

    posePath.header.stamp = ros::Time::now();
    posePath.header.frame_id = "odom";
    pubPosePath.publish(posePath);
  }
};
