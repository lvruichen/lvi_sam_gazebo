...
public:
    ...
    TransformFusion()
    {
        if (lidarFrame != baselinkFrame)
        {
            try
            {
                tfListener.waitForTransform(lidarFrame, baselinkFrame,
                                            ros::Time(0), ros::Duration(3.0));
                tfListener.lookupTransform(lidarFrame, baselinkFrame,
                                           ros::Time(0), lidar2Baselink);
            }
        ...
        subLaserOdometry = nh.subscribe<nav_msgs::Odometry>(
            "/ground_truth/state", 5, &TransformFusion::lidarOdometryHandler,
    ...
    void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        Eigen::Affine3f imuOdomAffineLast =
            lidarOdomAffine * imuOdomAffineIncre;
        float x, y, z, roll, pitch, yaw;
        pcl::getTranslationAndEulerAngles(imuOdomAffineLast, x, y, z, roll,
                                          pitch, yaw);
        ...
        if (imuTime - last_path_time > 0.1)
        {
            ...
            while (!imuPath.poses.empty() &&
                   imuPath.poses.front().header.stamp.toSec() <
                       lidarOdomTime - 5.0)
...
public:
    ...
    // IMUPreintegration接收"lio_sam/mapping/odometry_incremental"消息，然后使用因子图优化两个该消息之间的imu积分量，得到优化的bias。
    // 每一次来新的imu原始数据。利用优化后的imu的bias和预积分得到imu里程计，发布和imu频率一样的imu_odometry_incremental。
    // 第一步接受消息时。每100次清空因子图，优化变量
    IMUPreintegration()
    {
        ...
        imuIntegratorImu_ = new gtsam::PreintegratedImuMeasurements(
            p, prior_imu_bias);  // setting up the IMU integration for IMU
                                 // message thread
    ...
    void odometryHandler(const nav_msgs::Odometry::ConstPtr& odomMsg)
    {
        ...
        // 0. initialize system
        // 初始化系统的时候把之前的imuQueOpt给清空了
        // 初始化的时候利用PVQ把bias给优化了一次
        if (systemInitialized == false)
        {
            ...
            {
                if (ROS_TIME(&imuQueOpt.front()) <
                    currentCorrectionTime - delta_t)
                {
        ...
        // 每100次的时候把之前的因子图清空，把之前优化的初值给新的因子图
        if (key == 100)
        {
        ...
        while (!imuQueOpt.empty())
        {
            ...
            {
                double dt = (lastImuT_opt < 0) ? (1.0 / 100.0) :
        ...
        if (!imuQueImu.empty())
        {
            ...
            for (int i = 0; i < (int)imuQueImu.size(); ++i)
            {
                ...
                double imuTime = ROS_TIME(thisImu);
                double dt =
                    (lastImuQT < 0) ? (1.0 / 100.0) : (imuTime - lastImuQT);
    ...
    void imuHandler(const sensor_msgs::Imu::ConstPtr& imu_raw)
    {
        ...
        double imuTime = ROS_TIME(&thisImu);
        double dt =
            (lastImuT_imu < 0) ? (1.0 / 100.0) : (imuTime - lastImuT_imu);
        ...
        odometry.pose.pose.orientation.w =
            lidarPose.rotation().toQuaternion().w();
