...
public:
  ...
  TransformFusion() {
    ...
    subLaserOdometry = nh.subscribe<nav_msgs::Odometry>(
        "/husky_velocity_controller/odom", 5,
  ...
  void imuOdometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg) {
    ...
    if (imuTime - last_path_time > 0.1) {
      ...
      while (!imuPath.poses.empty() &&
             imuPath.poses.front().header.stamp.toSec() < lidarOdomTime - 5.0)
...
public:
  ...
  IMUPreintegration() {
    ...
    correctionNoise2 = gtsam::noiseModel::Diagonal::Sigmas(
        (gtsam::Vector(6) << 1, 1, 1, 1, 1, 1).finished()); // rad,rad,rad,m,
                                                            // m, m
  ...
  bool failureDetection(const gtsam::Vector3 &velCur,
    ...
    if (ba.norm() > 1.0 || bg.norm() > 1.0) {
      ROS_WARN("Large bias, reset IMU-add!");
      return true;
    }
