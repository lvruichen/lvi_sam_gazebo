...
POINT_CLOUD_REGISTER_POINT_STRUCT(
    ...
    (float, x, x)(float, y, y)(float, z, z)(float, intensity, intensity)(
        float,
        roll,
        roll)(float, pitch, pitch)(float, yaw, yaw)(double, time, time))
...
public:
    ...
    pcl::PointCloud<PointType>::Ptr laserCloudCornerLast;   // corner feature set
                                                            // from
                                                            // odoOptimization
    pcl::PointCloud<PointType>::Ptr laserCloudSurfLast;     // surf feature set
                                                            // from odoOptimization
    pcl::PointCloud<PointType>::Ptr laserCloudCornerLastDS; // downsampled
                                                            // corner feature
                                                            // set from
                                                            // odoOptimization
    pcl::PointCloud<PointType>::Ptr laserCloudSurfLastDS;   // downsampled surf
                                                            // feature set from
                                                            // odoOptimization
    ...
    std::vector<PointType> laserCloudOriCornerVec; // corner point holder for
                                                   // parallel computation
    std::vector<PointType> coeffSelCornerVec;
    std::vector<bool> laserCloudOriCornerFlag;
    std::vector<PointType> laserCloudOriSurfVec; // surf point holder for
                                                 // parallel computation
    ...
    mapOptimization() {
        ...
        pubKeyPoses = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/mapping/"
                                                             "trajectory",
                                                             1);
        pubLaserCloudSurround = nh.advertise<sensor_msgs::PointCloud2>("lio_"
                                                                       "sam/"
                                                                       "mapping"
                                                                       "/map_"
                                                                       "global",
        ...
        pubLaserOdometryIncremental = nh.advertise<nav_msgs::Odometry>("lio_"
                                                                       "sam/"
                                                                       "mapping"
                                                                       "/odomet"
                                                                       "ry_"
        ...
        srvSaveMap = nh.advertiseService(
            "lio_sam/save_map", &mapOptimization::saveMapService, this);

        pubHistoryKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/"
                                                                     "mapping/"
                                                                     "icp_loop_"
                                                                     "closure_"
                                                                     "history_"
                                                                     "cloud",
                                                                     1);
        pubIcpKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/"
                                                                 "mapping/"
                                                                 "icp_loop_"
                                                                 "closure_"
                                                                 "corrected_"
        ...
        pubRecentKeyFrames = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/"
                                                                    "mapping/"
                                                                    "map_local",
                                                                    1);
        pubRecentKeyFrame = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/"
                                                                   "mapping/"
                                                                   "cloud_"
                                                                   "registered",
                                                                   1);
        pubCloudRegisteredRaw = nh.advertise<sensor_msgs::PointCloud2>("lio_"
                                                                       "sam/"
                                                                       "mapping"
                                                                       "/cloud_"
                                                                       "registe"
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool saveMapService(lio_sam::save_mapRequest &req,
        ...
        for (int i = 0; i < (int)cloudKeyPoses3D->size(); i++) {
            *globalCornerCloud += *transformPointCloud(
                cornerCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
            *globalSurfCloud += *transformPointCloud(
                surfCloudKeyFrames[i], &cloudKeyPoses6D->points[i]);
            cout << "\r" << std::flush << "Processing feature cloud " << i
        ...
        *globalMapCloud += *globalSurfCloud;
        ...
        int ret = pcl::io::savePCDFileBinary(saveMapDirectory + "/GlobalMap."
                                                                "pcd",
                                             *globalMapCloud);
        res.success = ret == 0;
    ...
    void visualizeLoopClosure() {
        ...
        for (auto it = loopIndexContainer.begin();
             it != loopIndexContainer.end(); ++it) {
    ...
    void extractCloud(pcl::PointCloud<PointType>::Ptr cloudToExtract) {
        ...
        for (int i = 0; i < (int)cloudToExtract->size(); ++i) {
            if (pointDistance(cloudToExtract->points[i],
                              cloudKeyPoses3D->back())
            ...
            if (laserCloudMapContainer.find(thisKeyInd) != laserCloudMapContainer.end()) {
                ...
                *laserCloudCornerFromMap +=
                    laserCloudMapContainer[thisKeyInd].first;
                *laserCloudSurfFromMap +=
                    laserCloudMapContainer[thisKeyInd].second;
            } else {
        ...
        for (int i = 0; i < laserCloudCornerLastDSNum; i++) {
            ...
            if (pointSearchSqDis[4] < 1.0) {
                ...
                if (matD1.at<float>(0, 0) > 3 * matD1.at<float>(0, 1)) {
                    ...
                    float a012 = sqrt(
                        ((x0 - x1) * (y0 - y2) - (x0 - x2) * (y0 - y1)) * ((x0 - x1) * (y0 - y2) - (x0 - x2) * (y0 - y1)) + ((x0 - x1) * (z0 - z2) - (x0 - x2) * (z0 - z1)) * ((x0 - x1) * (z0 - z2) - (x0 - x2) * (z0 - z1)) + ((y0 - y1) * (z0 - z2) - (y0 - y2) * (z0 - z1)) * ((y0 - y1) * (z0 - z2) - (y0 - y2) * (z0 - z1)));
...
#pragma omp parallel for num_threads(numberOfCores)
    ...
    bool LMOptimization(int iterCount) {
        ...
        // need to cope with coordinate transformation lidar <- camera      ---
        // camera <- lidar x = z                ---     x = y y = x ---     y =
        // z z = y                ---     z = x roll = yaw           --- roll =
        // pitch pitch = roll         ---     pitch = yaw yaw = pitch --- yaw =
        // roll
        ...
        for (int i = 0; i < laserCloudSelNum; i++) {
            ...
            // in camera
            float arx =
                (crx * sry * srz * pointOri.x + crx * crz * sry * pointOri.y - srx * sry * pointOri.z) * coeff.x + (-srx * srz * pointOri.x - crz * srx * pointOri.y - crx * pointOri.z) * coeff.y + (crx * cry * srz * pointOri.x + crx * cry * crz * pointOri.y - cry * srx * pointOri.z) * coeff.z;
            ...
            float arz =
                ((crz * srx * sry - cry * srz) * pointOri.x + (-cry * crz - srx * sry * srz) * pointOri.y) * coeff.x + (crx * crz * pointOri.x - crx * srz * pointOri.y) * coeff.y + ((sry * srz + cry * crz * srx) * pointOri.x + (crz * sry - cry * srx * srz) * pointOri.y) * coeff.z;
            // camera -> lidar
    ...
    void scan2MapOptimization() {
        ...
        if (laserCloudCornerLastDSNum > edgeFeatureMinValidNum && laserCloudSurfLastDSNum > surfFeatureMinValidNum) {
            ...
            ROS_WARN("Not enough features! Only %d edge and %d planar features "
                     "available.",
    ...
    void transformUpdate() {
        if (cloudInfo.imuAvailable == true) {
            if (std::abs(cloudInfo.imuPitchInit) < 1.4) {
                ...
                tf::Matrix3x3(
                    transformQuaternion.slerp(imuQuaternion, imuWeight))
    ...
    void addGPSFactor() {
        ...
        if (cloudKeyPoses3D->points.empty())
            ...
            if (pointDistance(cloudKeyPoses3D->front(),
                              cloudKeyPoses3D->back())
                < 5.0)
                return;
        }
        ...
        while (!gpsQueue.empty()) {
            if (gpsQueue.front().header.stamp.toSec() < timeLaserInfoCur - 0.2) {
                ...
                Vector3 << max(noise_x, 1.0f), max(noise_y, 1.0f),
                    max(noise_z, 1.0f);
    ...
    void addLoopFactor() {
        ...
        for (int i = 0; i < (int)loopIndexQueue.size(); ++i) {
            ...
            gtsam::noiseModel::Diagonal::shared_ptr noiseBetween =
                loopNoiseQueue[i];
            gtSAMgraph.add(BetweenFactor<Pose3>(indexFrom, indexTo, poseBetween,
                                                noiseBetween));
        }
    ...
    void saveKeyFramesAndFactor() {
        ...
        // cout << "****************************************************" <<
        // endl; gtSAMgraph.print("GTSAM Graph:\n");
    ...
    void publishOdometry() {
        ...
        static nav_msgs::Odometry laserOdomIncremental; // incremental odometry
                                                        // msg
        static Eigen::Affine3f increOdomAffine;         // incremental odometry in
                                                        // affine
        if (lastIncreOdomPubFlag == false) {
            ...
            float x, y, z, roll, pitch, yaw;
            pcl::getTranslationAndEulerAngles(increOdomAffine, x, y, z, roll,
                                              pitch, yaw);
            if (cloudInfo.imuAvailable == true) {
                if (std::abs(cloudInfo.imuPitchInit) < 1.4) {
        ...
        if (pubSLAMInfo.getNumSubscribers() != 0) {
            if (lastSLAMInfoPubSize != cloudKeyPoses6D->size()) {
                ...
                *cloudOut += *laserCloudSurfLastDS;
                slamInfo.key_frame_cloud = publishCloud(
                    ros::Publisher(), cloudOut, timeLaserInfoStamp, lidarFrame);
                slamInfo.key_frame_poses =
                    publishCloud(ros::Publisher(), cloudKeyPoses6D,
                                 timeLaserInfoStamp, odometryFrame);
                pcl::PointCloud<PointType>::Ptr localMapOut(
                    new pcl::PointCloud<PointType>());
                *localMapOut += *laserCloudCornerFromMapDS;
                *localMapOut += *laserCloudSurfFromMapDS;
                slamInfo.key_frame_map =
                    publishCloud(ros::Publisher(), localMapOut,
                                 timeLaserInfoStamp, odometryFrame);
