...
public:
  ...
  FeatureExtraction() {
    ...
    pubCornerPoints = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/"
                                                             "feature/"
                                                             "cloud_corner",
                                                             1);
    pubSurfacePoints = nh.advertise<sensor_msgs::PointCloud2>("lio_sam/"
                                                              "feature/"
                                                              "cloud_"
                                                              "surface",
                                                              1);
  ...
  void laserCloudInfoHandler(const lio_sam::cloud_infoConstPtr &msgIn) {
    ...
    pcl::fromROSMsg(msgIn->cloud_deskewed,
                    *extractedCloud); // new cloud for extraction, the cloud has
                                      // been rendered by column and row
  ...
  void markOccludedPoints() {
    ...
    for (int i = 5; i < cloudSize - 6; ++i) {
      ...
      int columnDiff =
          std::abs(int(cloudInfo.pointColInd[i + 1] -
                       cloudInfo.pointColInd[i])); // pointColInd store the
                                                   // column idx of the cloud,
      // if ((depth2 - depth1) > 0.3) {
      //   cout << "has points" << i << endl;
      // }
      if (columnDiff < 10 || columnDiff > 1700) {
  ...
  void extractFeatures() {
    ...
    pcl::PointCloud<PointType>::Ptr surfaceCloudScanDS(
        new pcl::PointCloud<PointType>());
    // devide the space into six part and do feature extraction separately
    for (int i = 0; i < N_SCAN; i++) {
      ...
      for (int j = 0; j < 6; j++) {
        ...
        if (sp >= ep)
          continue;
        // sort the cloud by smoothness in erery six part from small to large
        ...
        // consider 1 as corner point and 0 and -1 as surface point. ps 0 means
        // the point does not fit any criterion, and surface cloud will be
        // downsampled
        for (int k = sp; k <= ep; k++) {
          if (cloudLabel[k] <= 0) {
